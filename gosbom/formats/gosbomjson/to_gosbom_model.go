...
func toGosbomModel(doc model.Document) (*sbom.SBOM, error) {
	...
	return &sbom.SBOM{
		...
		Relationships: warnConversionErrors(toGosbomRelationships(&doc, catalog, doc.ArtifactRelationships, idAliases)),
	}, nil
}
...
func toGosbomLicenses(m []model.License) (p []pkg.License) {
	for _, l := range m {
...
func toGosbomRelationships(doc *model.Document, catalog *pkg.Collection, relationships []model.Relationship, idAliases map[string]string) ([]artifact.Relationship, []error) {
	...
	for _, r := range relationships {
		gosbomRelationship, err := toGosbomRelationship(idMap, r, idAliases)
		if err != nil {
...
func toGosbomRelationship(idMap map[string]interface{}, relationship model.Relationship, idAliases map[string]string) (*artifact.Relationship, error) {
...
func toGosbomPackage(p model.Package, idAliases map[string]string) pkg.Package {
	...
	out := pkg.Package{
		...
		Licenses:     pkg.NewLicenseSet(toGosbomLicenses(p.Licenses)...),
		Language:     p.Language,
		Type:         p.Type,
		CPEs:         cpes,
		PURL:         p.PURL,
